...
function GameBoyAdvance() {
  ...
  this.throttle = 1; // This is rough, but the 2/3ms difference gives us a good overhead
...
GameBoyAdvance.prototype.turbo = function (stepCount = 1) {
  for (var i = 0; i < stepCount; ++i) {
    this.cpu.step();
    this.irq.updateTimers(this.cpu);
    this.step();
  }
};
...
GameBoyAdvance.prototype.updateGPRs = function () {
  for (var i = 0; i < 16; ++i) {
    this.cpu.gprs.children[i].textContent = hex(this.cpu.gprs[i]);
  }
};
